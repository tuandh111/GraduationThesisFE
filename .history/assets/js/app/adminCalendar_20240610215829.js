...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout) {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl,
                {
                    ...
                    eventClick: function(info) {
                        ...
                        var eventDetails = `
                                <li>Tên bác sĩ: ${info.event.extendedProps.doctor}</li>
                                <li>Tên bệnh nhân: ${info.event.extendedProps.patient}</li>
                                <li>Dịch vụ: ${info.event.extendedProps.service}</li>
                                <li>Thời gian bắt đầu: ${info.event.start.toLocaleString()}</li>
                                <li>Thời gian kết thúc: ${info.event.end.toLocaleString()}</li>
                            `;
                            document.getElementById('eventDetails').innerHTML = eventDetails;
                            var eventAlert = new bootstrap.Alert(document.getElementById('eventAlert'));
                            document.getElementById('eventAlert').classList.add('show');
                    }
                });
            calendar.render(); 
        }    
    }
    $scope.initializeUIComponents = function () {
        $('.select2').select2(
            {
                theme: 'bootstrap4',
            });
    ...
    $scope.initializeUppyUploader=function () {
