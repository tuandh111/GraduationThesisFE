app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout,$window) {
    ...
    $scope.initializeUIComponents = () => {
        // $scope.processDataDateRegistered().then(result => {
        //     regiseteredDates = result;
        //     console.log("regiseteredDates",regiseteredDates);
        // }).catch(error => {
        ...
        $('.drgpicker').daterangepicker(
            {
                ...
                isInvalidDate: function (date) {
                    ...
                    return regiseteredDates.includes(date.format('YYYY-MM-DD'));
                }
            }
            // function (start) {
    ...
    $scope.processDataDateRegistered = () => {
        return new Promise((resolve, reject) => {
            $http.get(url + '/doctor-schedule').then(response => {
                var dates=response.data.map(item=>item.date)
    ...
    $scope.crudDoctorSchedule = () => {
        ...
        $scope.validationForm = () => {
            ...
            dates=dates.map(date => moment(date, 'DD/MM/YYYY').format('YYYY-MM-DD'));
            console.log("regiseteredDates valid",regiseteredDates);
            // console.log("dates valid",dates);
            var hasOverlap = dates.some(date => registeredDates.includes(date));
            if (selecteShiftdValues == "" || selecteShiftdValues == null) {
                ...
                Swal.fire({
                    title: "Cảnh báo!",
                    html: "Ngày đang chọn đã đăng ký",
                    icon: "error"
                })
                valid = false
            }
        ...
        $scope.createDoctorSchedule = () => {
            ...
            //     var dataArray=$scope.generateScheduleArray(1,dates,shiftIds)
            //     //console.log("dataArray",dataArray);
            //     dataArray.forEach(data => {
            //         var requsetDataJSON = angular.toJson(data)
            //         console.log("requsetDataJSON",requsetDataJSON);
            //         $http.post(url+'/doctor-schedule',requsetDataJSON).then(response=>{
            //             Swal.fire({
            //                 title: "Thành công!",
            //                 html: "Đã đăng ký lịch làm việc cho ngày "+selectedDate,
            //                 icon: "success"
            //             }).then(() => {
            //                 // Sau khi thông báo thành công, làm mới trang
            //                 $window.location.reload(); // hoặc sử dụng $route.reload() nếu phù hợp với bạn
            //             });
            //             $scope.getDateRegisetered()
            //             $scope.resetForm()
            //         }).catch(error=>{
