...
app.controller('AdminListService', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.listServiceInfo = () => {
        $http.get(url + '/service').then(response => {
            ...
            if ($scope.listServiceFromDB.length > 0 && $scope.listServiceFromDB[0].hasOwnProperty('serviceId')) {
                $scope.listServiceFromDB.sort((a, b) => b.serviceId - a.serviceId);
            } else {
                console.error("Dữ liệu không có trường 'serviceId'.");
            }
