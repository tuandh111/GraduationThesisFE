...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service) {
    ...
    $scope.setDefaultStatus = function () {
        $scope.getListAppointmentStatus()
        console.log("$scope.listAppointmentStatusBD",$scope.listAppointmentStatusBD);
        angular.forEach($scope.listAppointmentStatusBD, function (s) {
            if (s.status.toLowerCase() === "đã xác nhận") {
                $scope.formAppointmentRequest.appointmentStatus = s.appointment_StatusId;
            }
        })
    }
