app.controller('AdminShiftController', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('#beginTimeOfShift').on('input', function () {
            $scope.$apply(function () {
                $scope.formTimeOfShift.beginTime = $(this).val();
            });
        });
        $scope.updateBeginTime = function() {
            // Cập nhật giá trị của biến beginTime trong formTimeOfShift
            // Lưu ý: Bạn có thể thêm các logic xử lý khác nếu cần thiết
            console.log("Selected begin time:", $scope.formTimeOfShift.beginTime);
        };
    }
    $scope.getListShift = () => {
