app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.initializeUIComponents = () => {
        // var haha =  $scope.getDateRegistered()
        // console.log("haha",haha);
        $scope.getDateRegistered().then(result => {
    ...
    $scope.getDateRegistered = () => {
        return new Promise((resolve, reject) => {
            $http.get(url + '/doctor-schedule').then(response => {
                resolve($scope.extractDatesFromData(response.data))
            }).catch(err => {
    ...
    $scope.extractDatesFromData = (datas) => {
        ...
        datas.forEach(data => {
            var dataFilter = data.filter(date => date!=null)
            dataFilter=dataFilter.map(date=>moment(date).format('YYYY-MM-DD'))
            dateRegisteredArray.push(dataFilter.date);
        });
