    ...
    $scope.processDoctorUnavailabilityAllDoctor = () => {
        ...
        return new Promise((resolve, reject) => {
            $http.get(url + '/doctorUnavailability').then((response) => {
                console.log("response okkkk",response.data);
                var filteredData = response.data.filter(item => {
                    return $scope.selectedDoctors.includes(item.appointment.doctor.doctorId)&&$scope.selectedDates.includes(item.appointment.appointmentDate)
                })
