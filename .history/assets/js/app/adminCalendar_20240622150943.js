...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('.drgpicker').on('apply.daterangepicker', function(ev, picker) {
            ...
            //alert('Apply button clicked! Selected date: ' + selectedDate);
    ...
    $scope.getListDoctorSchedule = (date) => {
        ...
        var requsetDateJSON = angular.toJson(date)    
        //console.log("requsetDateJSON",{"date":requsetDateJSON});
        $http.get(url+"/doctor-schedule-by-date",requsetDateJSON,{headers:{'Accept': 'application/json, text/plain, */*'}}).then(response => {
           $scope.listDoctorScheduleDB= response.data 
