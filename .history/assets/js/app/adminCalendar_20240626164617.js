...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService) {
    ...
    $scope.timeOfShiftDB = (shiftId, dateSelected, doctorId) => {
        var convertedDate = TimezoneService.convertToTimezone(dateSelected.toDate(), defaultTimezone)
        var params = {
            shiftId: shiftId,
            date: convertedDate,
            doctorId: doctorId
    ...
    $scope.saveAppointment = async () => {
        var appointmentDate = $scope.formAppointmentRequest.appointmentDate.toDate();
