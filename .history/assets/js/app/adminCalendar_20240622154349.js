...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.convertDateToISO = (date) => {
        // Chuyển đổi ngày từ định dạng DD/MM/YYYY sang đối tượng moment với múi giờ địa phương
        var momentDate = moment(date, 'DD/MM/YYYY');

        // Thiết lập thời gian cụ thể nếu cần (ở đây là giữ thời gian mặc định là 00:00:00.000)
        momentDate.set({ hour: 0, minute: 0, second: 0, millisecond: 0 });

        // Chuyển đổi đối tượng moment sang định dạng ISO mà không thay đổi múi giờ
        var isoDate = momentDate.format('YYYY-MM-DDTHH:mm:ss.SSSZ');

        console.log("isoDate", isoDate);  // Output: 2024-06-25T00:00:00.000+00:00
        return isoDate;
    };
    $scope.getListDoctorSchedule = (date) => {
        console.log("date", date)
        var convertDateToISO = $scope.convertDateToISO(date)
        console.log("convertDateToISO", convertDateToISO);
    }

    $scope.getListAppointmentStatus = () => {
        $http.get(url + '/appointment-status').then(respone => {
            $scope.listAppointmentStatusDB = respone.data
            console.log(" $scope.listDentalStaffDB", $scope.listDentalStaffDB);
        }).catch(err => {
            console.log("Error", err);
        })
    }
