app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.initializeUIComponents = () => {
        var disabledDates = ['2024-06-23', '2024-06-24', '2024-06-28'];   
        $('.select2').select2(
            {
                theme: 'bootstrap4',
            });
    ...
    $scope.getListDoctorSchedule = () => {
        $http.get(url + '/doctor-schedule').then(respone => {
            $scope.listDoctorScheduleDB = respone.data
            //console.log("$scope.listDoctorScheduleDB",$scope.listDoctorScheduleDB);
            dateRegisterd=$scope.extractDatesFromData($scope.listDoctorScheduleDB)
            console.log("dateRegisterd",dateRegisterd);
        })
    }

    $scope.extractDatesFromData = (datas) => {
        var dateRegisteredArray = [];
        datas.forEach(data => {
            dateRegisteredArray.push(data.date);
        });
        return dateRegisteredArray;
    }
