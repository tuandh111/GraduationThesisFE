app.controller('AdminListAppoinment', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route) {
    ...
    //     $scope.selectSubTab = (shiftId, doctorId, $event) => {
    ...
    //                     var beginTimeInSeconds = $scope.timeStringToSeconds(tos[1].beginTime);
    ...
    $scope.refresh = () => {
        $route.reload()
        // Đảm bảo rằng AngularJS đã được tải và khởi tạo
        angular.element(document).ready(function () {
            // Lấy đối tượng scope của phần tử chứa controller
            var scope = angular.element(document.querySelector('[ng-controller="YourControllerName"]')).scope();
            // Kiểm tra scope và hàm
            if (scope && scope.selectTab) {
