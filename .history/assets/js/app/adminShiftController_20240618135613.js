app.controller('AdminShiftController', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.initializeUIComponents = () => {
            ...
            $('.datetimes').daterangepicker(
                {
                    ...
                    //     format: 'M/DD hh:mm A'
                    // }
                    timePicker: true, // Cho phép chọn cả giờ
                    startDate: moment().startOf('day').add(moment(new Date().toTimeString().slice(0, 5), 'HH:mm').diff(moment().startOf('day'), 'minutes'), 'minutes'), // Ngày và giờ bắt đầu làm việc, dựa trên minTime của timepicker
                    endDate: moment().startOf('hour').add(32, 'hour'), // Ngày và giờ kết thúc làm việc, bắt đầu từ đầu giờ hiện tại và cộng thêm 32 giờ
                    locale: {
                        format: 'M/DD HH:mm', // Định dạng hiển thị ngày tháng giờ phút
                    }
                });
        }
        ...
        $('#beginTimeOfShift').on('change', function () {
            ...
            // Update your AngularJS model if necessary
            // $scope.formTimeOfShift.beginTime = selectedTime;
        });
        // $('#beginTimeOfShift').on('change', function () {
        //     var beginTime = $('#beginTimeOfShift').val();
        //     console.log("beginTime",beginTime);
        //     $('#endTimeOfShift').val('');
        //     if (beginTime) {
        //         var momentBeginTime = moment(beginTime, 'HH:mm');
        //         var endTimeOptions = [];

        //         for (var i = 0; i <= 21; i++) { 
        //             for (var j = 0; j < 60; j += 30) {
        //                 var endTime = moment().startOf('day').add(i, 'hours').add(j, 'minutes');
        //                 if (endTime.isSameOrAfter(momentBeginTime)) {
        //                     endTimeOptions.push(endTime.format('HH:mm'));
        //                 }
        //             }
        //         }


        //         var $endTimeSelect = $('#endTimeOfShift');
        //         $endTimeSelect.empty();
        //         $.each(endTimeOptions, function (i, option) {
        //             $endTimeSelect.append($('<option>').text(option));
        //         });


        //         if (endTimeOptions.length > 0) {
        //             $endTimeSelect.val(endTimeOptions[0]);
        //             $scope.formTimeOfShift.endTime = endTimeOptions[0];
        //         }
        //     }
        // });
    }
