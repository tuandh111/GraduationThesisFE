...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service) {
    ...
    //         appointmentStatusDefault=$scope.listAppointmentStatusBD.find((item) => item.status.toLowerCase() === 'đã xác nhận').appointment_StatusId            
    //     })
    // } 
    $scope.processAppointmentStatusDefault = () => {
        return new Promise((resolve, reject) => {
            $http.get(url + '/appointment-status').then(resp => {
                // $scope.listAppointmentStatusBD=resp.data
                var dataDefault = resp.data.find((item) => item.status.toLowerCase() === 'đã xác nhận').appointment_StatusId
                resolve(dataDefault)
            }).catch(err => {
    ...
    $scope.getAppointmentStatusDefault = () => {
        $scope.processAppointmentStatusDefault().then(rs => {
           appointmentStatusDefault = rs         
        })
    }
    $scope.initData = async  () => {
        await $scope.getAppointmentStatusDefault()
        console.log("appointmentStatusDefault from initData", appointmentStatusDefault == "");
