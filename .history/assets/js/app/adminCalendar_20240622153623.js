...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.getListDoctorSchedule = (date) => {
        // var requestDate = {
        //     date: moment(date, 'DD/MM/YYYY').utc().format('YYYY-MM-DDTHH:mm:ss.SSSZ')
        // }
        var isoDate =moment(date, 'DD/MM/YYYY').utc().format('YYYY-MM-DDTHH:mm:ss.SSSZ')
        var requestDate = { date: isoDate };
        var requestDateJSON = angular.toJson(requestDate);
        console.log("requestDateJSON",requestDateJSON);
    }
