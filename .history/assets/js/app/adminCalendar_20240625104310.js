...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('#formAppointmentRequestPatientId').on('change', function () {
            $timeout(function () {
                $scope.formAppointmentRequest.patientId = $('#formAppointmentRequestPatientId').val();
            });
        });

        $('#formAppointmentRequestAppointmentType').on('change', function () {
            $timeout(function () {
                // $scope.formAppointmentRequest.appointmentType = $('#formAppointmentRequestAppointmentType').val();
                var selectedVal = $('#formAppointmentRequestAppointmentType').val()
                //$scope.formAppointmentRequest.appointmentType=$scope.processSelect2Data(selectedVal)
                console.log("$scope.processSelect2Data(selectedVal)", $scope.processSelect2Data(selectedVal));
            });
        });
