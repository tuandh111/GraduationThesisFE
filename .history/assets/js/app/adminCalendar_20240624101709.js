        ...
        $http.get(url + '/doctor-schedule-by-date', { params: dateRequest }).then(response => {
            ...
            response.data.forEach(d => {
                if (d.doctor) {
                    ...
                    shiftMap.set(d.shift.shiftId,d.shift);
                }
            });

            // Chuyển Map thành mảng
            $scope.doctorDB = Array.from(doctorMap.values());
            $scope.shiftDB = Array.from(shiftMap.values());
            console.log("$scope.doctorDB", $scope.doctorDB);
            console.log("$scope.shiftDB",$scope.shiftDB);
        })
    }
