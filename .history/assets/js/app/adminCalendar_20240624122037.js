...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    // $scope.initializeTimepicker =  (item) => {
    //     $timeout(function () {
    //         $('.time-input').timepicker({
    //             'scrollDefault': 'now',
    //             'zindex': '9999', // Để sửa lỗi mở
    //             'timeFormat': 'H:i', // Định dạng 24 giờ
    //             'step': 15, // Bước nhảy của thời gian
    //             'disableTimeRanges': item.data // Sử dụng dữ liệu từ DB để vô hiệu hóa các khoảng thời gian
    //         });
    ...
    $scope.timeOfShiftDB = (ShiftId) => {
        return $http.get(url + '/time-of-shift-by-shift-id/' + ShiftId)
            ...
            .catch(function (error) {
                console.error('Error fetching time of shift:', error);
                throw error; // Ném lỗi nếu có lỗi xảy ra
            });
    }

    $scope.getTimeOfShift = (shiftId) => {
        //$scope.timeOfShiftDB lúc này là 1 Promise 
        $scope.timeOfShiftDB(shiftId)
            .then(function (data) {
                // Lưu dữ liệu từ timeOfShiftDB vào mỗi item
                var item = $scope.shiftDB.find(function (item) {
                    return item.shiftId === shiftId;
                });
                if (item) {
                    item.timeOfShiftData = data; // Gán dữ liệu từ server vào item
