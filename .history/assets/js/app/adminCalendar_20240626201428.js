                ...
                var calendar = new FullCalendar.Calendar(calendarEl,
                    {
                        ...
                        eventClick: function (info) {
                            ...
                            var isoStringStart = info.event.start.toISOString();
                            var timePartStart = isoStringStart.split('T')[1]; 
                            var timeStart = timePartStart.slice(0, 5);

                            var isoStringEnd = info.event.end.toISOString();
                            var timePartEnd = isoStringEnd.split('T')[1]; 
                            var timeEnd = timePartEnd.slice(0, 5);

                            var eventDetails =
                                '<strong>Tên bác sĩ:</strong> ' + info.event.extendedProps.doctor + '<br>' +
                                '<strong>Bệnh nhân:</strong> ' + info.event.title + '<br>' +
                                '<strong>Thời gian bắt đầu:</strong> ' + info.event.start.toISOString() + '<br>' +
                                '<strong>Thời gian kết thúc:</strong> ' + info.event.end.toISOString();
