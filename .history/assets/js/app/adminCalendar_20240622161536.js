...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.getListDoctorSchedule = (date) => {
         var reqDate =moment(date,"DD/MM/YYYY").format("YYYY-MM-DD")
         var requestDate={date:reqDate}
         var requsetDateJSON = angular.toJson(requestDate)
         console.log("requsetDateJSON",requsetDateJSON);
         $http.get(url +'/doctor-schedule-by-date',requsetDateJSON,{ headers: headers },params: { date: dateParam } ).then(respone => {
            $scope.listDoctorScheduleDB = respone.data;
            console.log("$scope.listDoctorScheduleDB");
         })
