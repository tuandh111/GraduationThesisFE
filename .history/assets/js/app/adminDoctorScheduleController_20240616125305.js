app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location) {
    ...
    $scope.initializeDraggable = function() {
        $('.drag-time-shift').draggable({
            appendTo: 'body',
            helper: 'clone'
        });
    ...
    // Function to initialize droppable behavior
    $scope.initializeDroppable = function() {
        $('#dropzone-time-shift').droppable({
            activeClass: 'active-time-shift',
            hoverClass: 'hover-time-shift',
            accept: ":not(.ui-sortable-helper)", // Reject clones generated by sortable
    ...
    // Initialize drag and drop when the controller is ready
    $scope.initializeDraggable();
    $scope.initializeDroppable();

    // Options for ng-drop directive
    $scope.dropOptions = {
        onDrop: function(event, ui) {
            // Handle drop event here if needed
