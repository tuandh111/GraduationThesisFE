app.controller('AdminDoctorController', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.initializeUIComponents = () => {
        ...
        $scope.$watch('listDegreesTypeDB', function(newVal, oldVal) {
            if (newVal !== oldVal) {
                $('#degreesId').select2({
                    theme: 'bootstrap4'
                });
        ...
        // If the model changes, update Select2
        $scope.$watch('formDoctor.degrees', function(newVal, oldVal) {
            if (newVal !== oldVal) {
                $('#degreesId').val(newVal).trigger('change');
            }
        });
    
        // Listen to the change event of Select2
