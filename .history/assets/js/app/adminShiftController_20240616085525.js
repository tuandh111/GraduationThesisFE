app.controller('AdminShiftController', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.crudshift = () => {
        ...
        $scope.editShift = (shift,$event) => {
            ...
            $event.preventDefault()          
            if (shift != null) {
                $scope.formshift = angular.copy(shift);
            }           
            const firstTabButtonCreate = document.getElementById('form-tab-shift');
        ...
        $scope.createshift = () => {           
            ...
            $http.post(url + '/shift', requsetshiftJSON).then(respone => {
                ...
                $scope.resetForm()
                $scope.getListshift()
                const secondTabButtonCreate = document.getElementById('list-tab-shift');
        ...
        $scope.deleteshift = (shift, $event) => {
            ...
            Swal.fire({
                text: "Bạn có muốn xóa Bác sĩ "+shift.fullName+" ?",
                icon: 'warning',
