...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service) {
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('.drgpicker').on('apply.daterangepicker', function (ev, picker) {
            ...
            $scope.formAppointmentRequest.appointmentDate = picker.startDate
            console.log("picker.startDate",$scope.formAppointmentRequest);
            //alert(selectedDate)
        });
    ...
    $scope.saveAppointment = async () => {
       ...
       // $scope.formAppointmentRequest.appointmentDate = moment(appointmentDate, 'DD/MM/YYYY').toDate()
        $scope.formAppointmentRequest.appointmentDate = moment(appointmentDate, 'DD/MM/YYYY').format('YYYY-MM-DD HH:mm:ss')
