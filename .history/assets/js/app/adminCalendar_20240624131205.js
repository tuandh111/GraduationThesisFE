...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.initializeTimepicker =  (shiftId,data) => {
        console.log("data form initializeTimepicker",data);
        $timeout(function () {
            $('#start-time-'+shiftId).timepicker({
                'scrollDefault': 'now',
                'zindex': '9999', // Để sửa lỗi mở
                'timeFormat': 'H:i', // Định dạng 24 giờ
                'step': 15, // Bước nhảy của thời gian
                'disableTimeRanges': data // Sử dụng dữ liệu từ DB để vô hiệu hóa các khoảng thời gian
            });
    ...
    $scope.getTimeOfShift = (shiftId) => {
        ...
        $scope.timeOfShiftDB(shiftId)
            .then(function (data) {
                console.log("data by "+shiftId,data);
                $scope.initializeTimepicker(shiftId,data)
            })
            .catch(function (error) {
