...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service) {
    ...
    $scope.setDefaultStatus = function () {
        angular.forEach($scope.listAppointmentStatusBD, function (status) {
            if (status.status.toLowerCase === "đã xác nhận") {
                $scope.formAppointmentRequest.appointmentStatus = status.appointment_StatusId;
            }
        })
    }
    $scope.initData = () => {
