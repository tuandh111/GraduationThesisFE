...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service) {
    ...
    //         $http.post(url, '/appointment', appointmentRequestJSON).then(response => {
    ...
    //             var doctorUnavailabilityRequestJSON = angular.fromJson($scope.formDoctorUnavailabilityResponse)
    //             $http.post(url, '/doctorUnavailability', doctorUnavailabilityRequestJSON).then(res => {
    //                 step = 2
    //             }).catch(error => {
    //                 console.log("Error: " + error);
    //             })
    ...
    //             var appointmentRecordRequestJSON = angular.fromJson($scope.formAppointmentPatientRecordRequest)
    //             $http.post(url, '/appointment-patient-record', appointmentRecordRequestJSON).then(resp => {
    ...
    $scope.saveAppointment = async () => {
        var appointmentDate = $scope.formAppointmentRequest.appointmentDate;
        $scope.formAppointmentRequest.appointmentDate = moment(appointmentDate, 'DD/MM/YYYY').toDate()
        ...
        if (valid) {
            try {
                ...
                // Tạo lịch làm việc bác sĩ
                $scope.formDoctorUnavailabilityRequest.date = $scope.getDataAfterPostAppointmentSuccess.appointmentDate;
                $scope.formDoctorUnavailabilityRequest.doctorId = $scope.getDataAfterPostAppointmentSuccess.doctor==nul?null:$scope.getDataAfterPostAppointmentSuccess.doctor.doctorId
