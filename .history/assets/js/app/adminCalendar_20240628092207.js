...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.allShiftsHidden = (shiftId) => {
        ...
        console.log("$scope.listTOS allShiftsHidden", $scope.listTOS);
        $scope.checkShiftsHidden = $scope.listTOS.length === 0 || $scope.listTOS.every((tos) => !$scope.isVisible(tos[1].beginTime)&&tos[1].shift.shiftId === shiftId);
        console.log("$scope.checkShiftsHidden",$scope.checkShiftsHidden);
    };
