app.controller('AdminDoctorController', function ($scope, $http, $rootScope, $location, $timeout) {
    ...
    $scope.crudDoctor = () => {
        ...
        $scope.editDoctor = (doctor,$event) => {
            ...
            if (doctor != null) {
                $scope.formDoctor = angular.copy(doctor);
                console.log("editDoctor",  $scope.formDoctor);
               // $scope.formDoctor.gender = service.serviceType ? service.serviceType.service_TypeId : -1
            }
            // $scope.formDoctor = {
            //     image: doctor.image,
